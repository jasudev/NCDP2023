...
struct PGIntroduction3: View {
    ...
    let items = [
        ...
        TitleSetData(title: "배포 대상 제한",
                     subtitle: "SwiftUI는 최소 버전 이상만 지원, 이전 버전의 운영 체제를 사용하는 경우에는 SwiftUI를 사용할 수 없음")]
    ...
    private func requirementsView() -> some View {
        HStack(alignment: .center) {
            ...
            VStack(alignment: .leading) {
                ...
                if indexValue >= 7 {
                    VStack(alignment: .leading, spacing: 8) {
                        Text("- 지난 4년 동안 도입된 기기의 81%가 iOS 16를 사용.")
                        Text("- iOS 15 미만은 4%에 불과.")
                    }
            ...
            if indexValue >= 6 {
                Spacer()
                PieChartView(title: "iOS 버전별 사용 현황", percents: [81, 15, 4], names: ["iOS 16", "iOS 15", "이전 버전"], formatter: {value in String(format: "%.0f", value)}, colors: [Color.blue, Color.green, Color.orange, Color.pink, Color.red], backgroundColor: .clear, widthFraction: 0.55, innerRadiusFraction: 0.45)
                    .padding(7.0)
